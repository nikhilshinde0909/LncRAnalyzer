...
LncFinder_codpot = {
    output.dir=summary_dir
    from("Putative.lnc_NPCTs.lncfinder.TSV") produce("LncFinder_codpot.TSV"){
        exec "sed 1,1d $input | cut -f 1,3 > $output"
    }
}

ROC_curve = {
    output.dir=summary_dir
    from("LncRAnalyzer-Lncs-intersect.txt","LncRAnalyzer-NPCTs-intersect.txt","FEELnc_codpot.TSV","CPAT_codpot.TSV","CPC2_codpot.TSV","RNAsamba_codpot.TSV","lgc_codpot.TSV","LncFinder_codpot.TSV") produce("LncRAnalyzer-ROC.log") {
        exec "$Rscript $lnc_roc_script $input1 $input2 $input3 $input4 $input5 $input6 $input7 $input8 > $output"
    }
}

LncRAnalyzer_ROC = segment{
    shuffle_CPS + intergenic_CPS + FEELnc_CPS + 
    CPC2_codpot + CPAT_codpot + RNAsamba_codpot + lgc_codpot + LncFinder_codpot + ROC_curve
}
