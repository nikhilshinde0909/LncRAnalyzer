...
FastP = {
	def input_fatsq_options = ""
	if(reads_R2=="")
	input_fatsq_options = "-i "+input
	else
	input_fastq_options = "-i "+input1+" -I "+input2
	output.dir=fastp_dir
	if(reads_R2=="")
	produce(branch.name+".fastq.gz",branch.name+".html",branch.name+".json"){
        exec "$fastp --thread $threads $input_fatsq_options -o $output1 --html $output2 --json $output3"
	}
        else
        produce(branch.name+"_1.fastq.gz", branch.name+"_2.fastq.gz",branch.name+".html",branch.name+".json"){
        exec "$fastp --thread $threads $input_fatsq_options -o $output1 -O output2 --html $output3 --json $output4"
	}
}
