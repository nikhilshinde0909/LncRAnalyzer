...
get_bed_files = {
	output.dir=slncky_dir
	from("Putative.lnc_NPCTs.gtf","LncRAnalyzer-Lncs-intersect.gtf") produce("Putative-lnc-nptcs.bed","LncRAnalyzer-Lncs-intersect.bed"){
	exec """
	$gffread $input1 --bed -o ${output.dir}/temp.bed ;
	cut -f1-12 ${output.dir}/temp.bed > $output1 && rm ${output.dir}/temp.bed ;
	$gffread $input2 --bed -o ${output.dir}/temp.bed ;
	cut -f1-12 ${output.dir}/temp.bed > $output2 && rm ${output.dir}/temp.bed 
	"""
	  }
}
...
ortholog_search = {
	output.dir = slncky_dir
	if (liftover != ""){
	from("annotation.config","LncRAnalyzer-Lncs-intersect.bed") produce(rel_sp_name+".orthologs.top.txt"){
        exec """
        source ${Activate} cpc2-cpat-slncky ;
...
slncky_run = segment { ref_genome_bed + fasta_index + annotation_config + get_bed_files + run_slncky + ortholog_search }
