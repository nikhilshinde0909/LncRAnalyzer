...
10. The pipeline is ready for execution \
Run the following command to print help
```
bpipe run -n ${threads} ~/Path_to_LncRAnalyzer/Main.groovy --help
```

11. Search for supporting species and configure it in the data.groovy
```
bpipe run -n ${threads} ~/Path_to_LncRAnalyzer/Main.groovy --supporting_species
```

12. Execute pipeline with RNA-seq datasets as follows
```
